...
class BidRepositoryImpl implements BidRepository {
  ...
  @override
  Future<Either<Failure, CreateBidResponseModel>> createAndUpdateBid(
      CreateBidEntity createBidEntity) async {
    try {
      // create bid
      if (createBidEntity.bidId == null) {
        debugPrint(
            'Call create bid createAndUpdateBid repository ${createBidEntity.toJson()}');
        return await bidRemoteDataSource.createBid(
            createBidEntity.bidStatus, createBidEntity);
      }
      // update bid
      else {
        return await bidRemoteDataSource.updateBid(
            createBidEntity.bidStatus, createBidEntity);
      }
    } catch (e) {
